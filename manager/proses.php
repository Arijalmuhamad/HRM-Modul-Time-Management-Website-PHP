...
if (isset($_GET['absen'])) {
	if($_GET['absen']==1){
		...
		if ($statement = $conn->prepare($sql)) {
			...
			if ($statement->execute()) {
				// Absen sukses
				$conn->close();
				header("location:../manager/absen.php?ab=1");
			} else {
				header("location:../manager/absen.php?ab=1");
			}
		}else {
	...
	//	$sql = "UPDATE data_absen SET jam_klr=?, st_jam_klr=? WHERE npp='$id_user' AND id_tgl='$day_tgl' AND id_bln='$id_bln'";
		...
		//if ($statement= $conn->prepare($sql)) {
			...
			if($query) {
				//$conn->close();
				header("location:../manager/absen.php?ab=2");
	...
	if (!isset($_SESSION['pb'])) {
		...
		if ($type==="in") {
			...
			if ($statement = $conn->prepare($query)) {
				...
				if ($statement->execute()) {
					// sukses update
					header("location:../manager/absen&ab=1");
				}else{
					//gagal update
					header("location:../manager/absen&ab=2");
				}
				$conn->close();
			} else {
				header("location:../manager/absen&ab=2");
			...
			if ($statement = $conn->prepare($query)) {
				...
				if ($statement->execute()) {
					// sukses update
					header("location:../pegawai/absen&ab=1");
				}else{
					//gagal update
					header("location:../pegawai/absen&ab=2");
				}
				$conn->close();
			} else {
				header("location:../pegawai/absen&ab=2");
		...
		for($i=0; $i < $count_id; $i++)
		{
		    ...
		    if ($type==="in") {
				...
				if ($statement = $conn->prepare($query)) {
					...
					header("location:../pegawai/absen&ab=6");
				}
	...
	if (!isset($_SESSION['pb'])) {
		...
		if ($type==="in") {
			...
			if ($statement = $conn->prepare($query)) {
				...
				if ($statement->execute()) {
					// sukses update
					header("location:../pegawai/absen&ab=3");
				}else{
					//gagal update
					header("location:../pegawai/absen&ab=2");
				}
